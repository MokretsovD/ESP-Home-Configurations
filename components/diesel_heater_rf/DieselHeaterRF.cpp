...
void DieselHeaterRF::sendCommand(uint8_t cmd, uint32_t addr, uint8_t numTransmits) {
  ...
  // CCA disabled (CCA_MODE=0): transmit unconditionally. With CCA_MODE=3 the CC1101 silently
  // refuses STX if RSSI exceeds threshold (e.g. heater's own broadcast, any ambient RF).
  // Disabling it guarantees our burst fires regardless of channel state.
  ...
  writeReg(0x17, 0x00); // MCSM1: CCA_MODE=0, TXOFF_MODE=IDLE
...
void DieselHeaterRF::initRadio() {
  ...
  writeReg(0x17, 0x00); // MCSM1: CCA_MODE=0 (no CCA), TXOFF_MODE=IDLE
